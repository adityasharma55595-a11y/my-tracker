...
# === Allowed statuses ===
$allowedStatuses = @(
    "DELIVERED", "DISPATCHED", "OUT FOR DELIVERY", "IN TRANSIT",
    "RECEIVED AT DESTINATION", "RECEIVED", "NOT DELIVERED", "ATTEMPTED", "RTO"
)
    ...
    foreach ($order in $response.orders) {
        ...
        foreach ($fulfillment in $order.fulfillments) {
            foreach ($awb in $fulfillment.tracking_numbers) {
                ...
                $dtdcBody = @{ trkType = "cnno"; strcnno = $awb; addtnlDtl = "Y" } | ConvertTo-Json -Compress
